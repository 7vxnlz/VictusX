...
| NvAPIWrapper LGPL packaging disposition | Blocked | Choose and verify one path: distribute NvAPIWrapper in replaceable external form, obtain documented approval for single-file LGPL compliance, or remove/replace the NvAPIWrapper-dependent GPU telemetry path before distribution. | [Runtime dependency review](runtime-dependency-license-review.md), [Package contents inspection](preview-package-contents-inspection.md) |
