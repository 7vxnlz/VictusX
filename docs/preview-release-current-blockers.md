...
| `THIRD-PARTY-NOTICES.md` release readiness | Blocked | The file exists only as a source-level draft. It is not release-ready until dependency notices, upstream attribution, icon attribution, and final artifact matching are reviewed. | [Third-Party Notices draft](../THIRD-PARTY-NOTICES.md) |
...
| Final package contents inspection | Blocked | No artifact exists to inspect. Future inspection must verify notices/license, launcher arguments, absence of developer-only logs/device captures, and signing/checksum evidence. | [Packaging readiness audit](windows-packaging-readiness-audit.md) |
