...
| Self-contained .NET runtime notices | Blocked | Source-side packaging intent is documented, but the exact runtime and Windows Desktop runtime components, native runtime files, matching runtime license/notices, and final artifact hash/reference must be verified against a future release candidate. | [.NET runtime notice checklist](dotnet-runtime-notice-checklist.md), [Package contents inspection](preview-package-contents-inspection.md) |
...
- Self-contained .NET runtime notices matched to the release candidate using the [.NET runtime notice checklist](dotnet-runtime-notice-checklist.md), and NvAPIWrapper LGPL packaging/relinking disposition completed.
